...
rd_w = [34,96]
rd = [
 ["What this is","Great Eastern Stream 3 CDP business use-case bank, version 2. Plain-English business use cases for B1, B3, C3, re-structured after the working-team review. This is the bank that everything downstream is built on (Data Cloud mapping, then prioritization, then the SI build)."],
 ["What changed in v2",""],
 ...
 ["GE journey confirmation","Confirm the January end-to-end journey stage names used here."],
 ["Counts","63 use cases total: B1 15, B3 19, C3 17, eligibility and claims 2, foundational 10."],
]
...
st_w = [7,22,40,20,40]
st = [
 ["1","Outreach","Paid and own media reaches the prospect","B1, B3","New stage added before awareness to hold media use cases"],
 ["2","Awareness","Prospect becomes aware of GE and the product","B1, B3","Ad-to-site creative continuity"],
 ["3","Exploration","Prospect explores on the landing page and site; existing customers may be recognised","B1, B3","Landing-page personalization and drop recovery sit here"],
 ["4","Quotation","Quote generated; B1 self-served, B3 via FA through Great Planner","B1, B3","Lead enrichment, routing, advisor meeting sit here"],
 ["5","KYC","Identity verification; Singpass in Singapore, guided KYC in Malaysia","B1, B3","Prefill reduces friction"],
 ["6","Eligibility check","Underwriting eligibility assessment","B1, B3","In the GE journey; light CDP coverage today, flagged for input"],
 ["7","Payment","Customer pays; card, PayNow, e-wallet, or FA-shared URL","B1, B3","Highest-urgency drop recovery, P0"],
 ["8","Issuance","Policy issued","B1, B3","Triggers onboarding"],
 ["9","Post-purchase","Onboarding, engagement, renewal, cross-sell and retention on the owned base","B1, B3, C3","C3 lives here; the largest opportunity"],
 ["10","Claims management","Claims experience and servicing","All","In the GE journey; light CDP coverage today, flagged for input"],
 ["11","Cross-journey (foundational)","Always-on CDP capabilities that underpin every stage","All","Data, identity, consent, arbitration, frequency, retrain"],
]
...
uc = [
 ...
 [J,"Channel execution","Payment","B1","B1-12","Payment-drop recovery (P0 urgency)","Payment drop is highest urgency; NBA fires channels 1-3 fast","Drop event (P0) + NBA + multichannel","Payment state, value, contact","Decisioning + Campaign Management","Stop leakage","Payment completion rate","Recovers near-won sales","P1"],
 [J,"Channel execution","Payment","B1","B1-13","Payment-method optimization","Offer card / PayNow / e-wallet by preference and market","Real-time personalization","Market, preference","Real-time Personalization","Convert","Payment success rate","Reduces payment friction","P2"],
 [J,"Onboarding","Post-purchase","B1","B1-14","Digital onboarding journey","On policy issue, fire welcome and activation sequence","Journey orchestration","Bind event, profile, channel pref","Campaign Management","Activate","Activation rate","Early value; builds the retention base","P1"],
 [J,"Data enablement","Post-purchase","B1","B1-15","Lifecycle handoff prospect to customer","Update lifecycle stage; set Account_Owner (or null if no FA); make eligible for C3","Profile update + workflow","Bind event, FA flag","CDP + CRM + Workflow","Retain and grow","Lifecycle data integrity","Enables future C3 lifetime value","P2"],
 # ---- B3 ----
 ...
 [J,"Channel execution","Payment","B3","B3-17","Payment-drop P0 with FA priority callback","High-value payment drop triggers an FA priority callback","Drop event (P0) + FA callback","Payment state, value, FA","Decisioning + CRM + Great Planner","Stop leakage","Payment completion rate","Recovers high-value sales","P1"],
 [J,"Onboarding","Issuance","B3","B3-18","Policy issuance and onboarding handoff","Email policy issuance; hand off to onboarding","Journey orchestration","Bind event","Campaign Management","Activate","Onboarding rate","Builds the retention base","P2"],
 [J,"Data enablement","Post-purchase","B3","B3-19","B3 to C3 lifecycle handoff","Account_Owner becomes the converting FA; future C3 triggers route to that FA via advisor continuity","Profile and ownership write","Bind, FA id","CDP + CRM","Retain and grow","Lifetime-value continuity","Strongest LTV motion in the model","P1"],
 ...
 # ---- C3 (post-purchase) ----
 [J,"Retention","Post-purchase","C3","C3-1","Multi-model propensity stack","Score the base for lapse (event on payment-failure, weekly otherwise), switch-intent (weekly), maturity rollover (90 days pre-maturity), MHIT fit (monthly), cross-sell per line (weekly), FA-acceptance (event)","Model stack in Decisioning / Studio","Base data, events, behaviour","AI/ML Engine + CDP","Retain and grow","Model coverage, precision","Surfaces high-ROI opportunities on owned base","P1"],
 [J,"Data enablement","Post-purchase","C3","C3-2","Score vector with SHAP reason codes","Each customer carries a vector of scores with reason codes, not a single label","Scoring + explainability","Features per customer","AI/ML Engine","Retain and grow","Explainability, action quality","Trust and better treatment selection","P1"],
 [J,"Channel execution","Post-purchase","C3","C3-3","NBA arbitration across firing models","When several models fire on one customer, resolve by value, urgency and fatigue limits; suppression prevents over-contact","NBA arbitration","Scores, value, fatigue","Decisioning","Retain and grow","Action precision, fatigue","Right single action per customer","P1"],
 [J,"Channel execution","Post-purchase","C3","C3-4","Advisor-continuity routing","CRM task defaults to the mapped FA (Account_Owner); grab-pool fallback after SLA via Flow","Routing + SLA flow","Account_Owner, SLA","CRM + Workflow","Convert","Actioned rate, SLA breach","Keeps relationship and recurring revenue","P1"],
 [J,"Channel execution","Post-purchase","C3","C3-5","Grab-pool fallback","Unactioned tasks reassigned after SLA breach","SLA timer + flow","Task status, SLA","Workflow + CRM","Stop leakage","Task leakage rate","Prevents dropped opportunities","P1"],
 [J,"Engagement","Post-purchase","C3","C3-6","NBA-selected engagement journey","Campaign journey indexed by persona x trigger x propensity x lifestage","Journey selection","Persona, trigger, propensity","Campaign Management","Pass-through","Engagement rate","Warms the customer before the FA","P1"],
 [J,"Engagement","Post-purchase","C3","C3-7","Authenticated 1:1 personalization","Logged-in onsite and in-app personalization across home tile, sidebar, offers and notifications","Real-time personalization (authenticated)","Login, Customer 360","Real-time Personalization","Pass-through","Onsite engagement, conversion","Lifts conversion on the owned base","P1"],
 [J,"Retention","Post-purchase","C3","C3-8","Loss-if-lapse calculator (1:1)","Render real coverage figures and dependants from Customer 360, not a generic illustration","Dynamic content from Customer 360","Coverage, dependants","CDP + Real-time Personalization","Retain and grow","Lapse-save rate","Prevents lapse; retains premium","P1"],
 [J,"Retention","Post-purchase","C3","C3-9","Comparison content vs current plan","Use the customer current plan as the baseline for switch defence","Dynamic content","Current plan data","Real-time Personalization + CDP","Retain and grow","Switch-save rate","Defends against competitor churn","P1"],
 [J,"Channel execution","Post-purchase","C3","C3-10","Non-engagement escalation","Non-engagement triggers an escalation path","Trigger + escalation","Engagement signals","Decisioning + Campaign Management","Stop leakage","Escalation effectiveness","Recovers silent at-risk customers","P2"],
 [J,"Engagement","Post-purchase","C3","C3-11","FA briefing for retention and cross-sell","Briefing pack (holdings, scores, reason codes, next-best-offer) in Sales Cloud / FA Mobile App","Briefing generation","Customer 360, scores","AI/ML Engine + CRM","Convert","FA prep, close rate","Effective retention conversations","P1"],
 [J,"Cross-sell / upsell","Post-purchase","C3","C3-12","Maturity rollover next-best-product (90 days)","Surface the next-best-product for policies maturing within 90 days","Rollover model + journey","Maturity dates, propensity","AI/ML Engine + Campaign Management","Retain and grow","Rollover capture rate","Retains maturing premium","P1"],
 [J,"Cross-sell / upsell","Post-purchase","C3","C3-13","MHIT cross-sell for coverage gaps","Identify coverage gaps and recommend MHIT","Gap model + next-best-offer","Holdings, gaps","AI/ML Engine + CRM","Retain and grow","MHIT cross-sell rate","Closes protection gaps; adds premium","P1"],
 [J,"Data enablement","Post-purchase","C3","C3-14","Closed-loop outcome capture","FA logs structured outcomes (interest, objection, follow-up, next step); streams to Data Cloud to refresh features and the NBA log","Structured capture + write-back","FA outcomes","CRM + CDP + AI/ML Engine","Retain and grow","Model lift, data quality","Compounding intelligence over time","P1"],
 [J,"Channel execution","Post-purchase","C3","C3-15","Wait-state management","Not-ready outcomes set calibrated waits (7-14 days lapse, 30-90 days cross-sell) with content rotation","Wait-state rules + rotation","Outcome, trigger type","Decisioning + Campaign Management","Retain and grow","Re-engage success, fatigue","Nurtures without over-contact","P2"],
 [J,"Channel execution","Post-purchase","C3","C3-16","Cool-down on conversion","On an adjacent-product purchase, suppress that product family during the cool-down window","Suppression rules","Purchase events","Decisioning + CDP","Retain and grow","Over-contact rate","Protects trust and relevance","P2"],
 [J,"Cross-sell / upsell","Post-purchase","C3","C3-17","Up-sell on life events","Recommend a higher coverage tier on a life-event signal","Event trigger + next-best-offer","Life-event signals","AI/ML Engine + Campaign Management","Retain and grow","Up-sell rate","Incremental premium","P2"],
 # ---- Foundational ----
...
out = "GE-Stream3-CDP-usecase-bank-v2.xlsx"
