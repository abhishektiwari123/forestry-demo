...
rd_w = [34,96]
rd = [
 ["What this is","Great Eastern Stream 3 CDP business use-case bank, version 2.2. Plain-English business use cases for B1, B3, C3, re-structured after the working-team review and aligned to the Project Clearwater GE journey. v2.2 adds a per-row reason column (why this stage and group). This is the bank that everything downstream is built on (Data Cloud mapping, then prioritization, then the SI build)."],
 ...
 ["What changed in v2.2 (this version)",""],
 ["Per-row reason column added","Added a 'Why this stage and group' column to the Journey use cases tab. Each row states why it sits on its GE stage (the triggering moment) and why it carries its use-case group (the CDP motion); judgement-call rows carry an extra note. See the Structure rationale tab for the column-level logic."],
...
uc_h = ["Tier","Use-case group","GE journey stage","Journey","ID","Business use case","How the stack helps (data -> trigger -> action)","Technical use case","Data inputs needed","Capability layer (vendor-neutral)","Funnel / value metric","Business KPI (the vector)","How it makes money / moves KPI","Priority","Why this stage and group"]
uc_w = [13,18,17,9,7,25,38,27,23,25,15,21,26,8,48]
...
# ----- Append a per-row "Why this stage and group" reason -----
# Reason = why it sits on this GE stage (the triggering moment) + why this use-case group (the CDP motion).
# Stage clause is composed from the stage; group clause from the group; an optional per-ID note adds specificity.
STAGE_WHY = {
 "Outreach":"sits at outreach because the trigger fires before the customer is aware, in paid or own media",
 "Awareness":"sits at awareness because it acts on the customer's first branded impression",
 "Exploration":"sits at exploration because the customer is actively researching on site",
 "Quotation":"sits at quotation because the trigger is the quote or lead moment",
 "KYC":"sits at KYC because the trigger is identity verification and account opening",
 "Eligibility check":"sits at eligibility because the trigger is the underwriting decision",
 "Payment / policy issuance":"sits at payment and issuance because the trigger is checkout or policy issue",
 "Post-purchase servicing":"sits at post-purchase because it acts on the owned customer after issuance",
 "Claims management":"sits at claims because the trigger is a claim event or status change",
 "Cross-journey":"is tagged cross-journey because it underpins every stage rather than one",
 "Rose, journey-wide":"is tagged journey-wide because the GE slide shows Rose running across the whole journey",
 "Skybranch, journey-wide":"is tagged journey-wide because the GE slide shows Skybranch running across the whole journey",
}
GROUP_WHY = {
 "Acquisition":"grouped as acquisition because the motion is sourcing new prospects or leads",
 "Onboarding":"grouped as onboarding because the motion is activating the new customer",
 "Engagement":"grouped as engagement because the motion is deepening interaction and personalization",
 "Retention":"grouped as retention because the motion is keeping the customer and protecting premium",
 "Cross-sell / upsell":"grouped as cross-sell because the motion is growing the relationship with more product",
 "Channel execution":"grouped as channel execution because the motion is delivering or routing the action",
 "Data enablement":"grouped as data enablement because it is foundational data, identity or consent work",
}
# Optional per-ID note for rows where the placement is a judgement call or GE-specific.
ID_NOTE = {
 "B1-8":"recovery is tagged by its mechanic (channel execution), though its intent is conversion",
 "B1-10":"recovery is tagged by its mechanic (channel execution), though its intent is conversion",
 "B1-12":"recovery is tagged by its mechanic (channel execution), though its intent is conversion",
 "B3-4":"placed at exploration because the drop it recovers happens on the landing page",
 "B3-9":"GE-specific four-tier routing, so grouped under channel execution",
 "B3-11":"orphan-lead handling is a GE Great Planner mechanic",
 "B3-19":"placed at post-purchase because the handoff to C3 happens after issuance",
 "C3-1":"C3 sits at post-purchase because it acts on the existing base",
 "GEN-1":"added so the eligibility stage is not empty in the GE journey",
 "GEN-2":"added so the claims stage is not empty in the GE journey",
 "GE-S9":"tagged data enablement and cross-journey because scam scanning runs across the journey",
 "GE-S12":"tagged data enablement because consent capture grows the addressable base",
 "F-11":"Rose is journey-wide on the GE slide, so it is foundational, not a single stage",
 "F-12":"Skybranch is journey-wide on the GE slide, so it is foundational, not a single stage",
}
for _row in uc:
    _grp, _stage, _id = _row[1], _row[2], _row[4]
    _s = STAGE_WHY.get(_stage, "placed at the stage where its trigger occurs")
    _g = GROUP_WHY.get(_grp, "grouped by its dominant CDP motion")
    _reason = f"It {_s}; {_g}."
    if _id in ID_NOTE:
        _reason += f" Note: {ID_NOTE[_id]}."
    _row.append(_reason)
...
# ===== Sheet: Structure rationale (column-level reasoning) =====
sr_h = ["Column","What it answers","Why it exists","Assignment rule","Caveat to flag"]
sr_w = [22,30,40,40,40]
sr = [
 ["Why two lenses","Why both a stage and a group column","No single axis does both jobs; outcome, segment and campaign views were each rejected in review","Stage is the client journey lens; group is the practitioner CDP-motion lens","They are complementary; each covers the other's blind spot"],
 ["A. Tier","Is it journey-level or foundational","Separates stage-specific use cases from always-on enablers (identity, consent, Rose, Skybranch)","Foundational where the use case underpins every stage rather than one","Foundational rows cannot sit on a single stage, which is why this split exists"],
 ["B. Use-case group","What kind of CDP work it is","Practitioner lens from the MSB taxonomy; reusable across journeys; rolls up by motion","Assigned by dominant business motion (acquisition, onboarding, engagement, retention, cross-sell, channel execution, data enablement)","Some rows are judgement calls; drop recovery is tagged by mechanic (channel execution) not intent; best-fit pending the official MSB grouping"],
 ["C. GE journey stage","Where in the customer journey it fires","Client-facing lens; maps to the Project Clearwater journey GE already owns; proves end-to-end coverage; the half business reviewers read","Placed at the stage where the triggering customer moment occurs","Payment and issuance merged to match the GE slide; outreach added before awareness as a Clearwater addition; journey-wide and cross-journey layers are tagged rather than forced onto a stage"],
 ["D. Journey","Which of B1, B3, C3 it serves","Ties the use case to the three board-priority journeys","Tagged to the journey whose flow it belongs to; All for foundational and shared","Some shared rows carry more than one journey"],
 ["Why this stage and group","Per-row defence of C and B","Makes the reasoning travel with each row, so the structure is defensible line by line","Composed from the row's own stage clause plus group clause, plus a note on judgement-call rows","If the official MSB grouping changes a group, the matching reason updates with it"],
]
...
sheets = [("Read me and change log", sheet_xml([rd_h]+rd, rd_w)),
          ...
          ("Structure rationale", sheet_xml([sr_h]+sr, sr_w)),
...
out = "GE-Stream3-CDP-usecase-bank-v2.2.xlsx"
